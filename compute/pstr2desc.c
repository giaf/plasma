 ...
 *
 * @generated from compute/pztr2desc.c, normal z -> s, Sat Jul 22 09:24:23 2017
 *
 **/
                      ...
                      plasma_request_t *request)
{
    ...
    for (int m = 0; m < A.mt; m++) {
        ...
        for (int n = n_start; n < n_end; n++) {
            ...
            float *f77 = &pA[(size_t)A.nb*lda*n + (size_t)A.mb*m];
            float *bdl = (float*)plasma_tile_addr(A, m, n);

            core_omp_slacpy(PlasmaGeneral, PlasmaNoTrans,
                            y2-y1, x2-x1,
                            &(f77[x1*lda+y1]), lda,
                            &(bdl[x1*A.nb+y1]), ldt,
                            sequence, request);
