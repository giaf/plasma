 ...
 *
 * @generated from compute/pzhetrf_aasen.c, normal z -> c, Sat Jul 22 09:24:14 2017
 *
 **/
#include <math.h>
                ...
                for (int n = 1; n < k+1; n++) {
                                     ...
                                     depend(inout:a2[0:ma2*na])
                    {
                        if (sequence->status == PlasmaSuccess) {
                            ...
                            core_cgeswp(PlasmaRowwise, view, k1, k2, ipiv, 1);
                        }
                    }
                }
                ...
                // -- symmetrically apply pivoting to trailing A -- //
                {
                                     ...
                                     depend(inout:a2[0:mnt2])
                    {
                        ...
                        for (int rank = 0; rank < num_swap_threads; rank++) {
                            #pragma omp task shared(barrier)
                            {
                                core_cheswp(rank, num_swap_threads, PlasmaLower, A, k1, k2, ipiv, 1, &barrier);
                            }
                        }
                        #pragma omp taskwait
                    }
