 ...
 *
 * @generated from compute/pzdesc2ge.c, normal z -> c, Sat Jul 22 09:24:17 2017
 *
 **/
                      ...
                      plasma_request_t *request)
{
    ...
    for (m = 0; m < A.mt; m++) {
        ...
        for (n = 0; n < A.nt; n++) {
            ...
            bdl = (plasma_complex32_t*)plasma_tile_addr(A, m, n);

            core_omp_clacpy(PlasmaGeneral, PlasmaNoTrans,
                            y2-y1, x2-x1,
                            &(bdl[x1*A.nb+y1]), ldt,
                            &(f77[x1*lda+y1]), lda,
                            sequence, request);
