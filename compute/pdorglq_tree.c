 ...
 *
 * @generated from compute/pzunglq_tree.c, normal z -> d, Sat Jul 22 09:24:15 2017
 *
 **/
...
#define Q(m, n) (double*)plasma_tile_addr(Q, m, n)
    ...
    for (int iop = num_operations-1; iop >= 0; iop--) {
        ...
        if (kernel == PlasmaGeKernel) {
            ...
            for (int m = j; m < Q.mt; m++) {
                ...
                core_omp_dormlq(PlasmaRight, PlasmaNoTrans,
                                mvqm, nvqk,
                                imin(mvaj, nvak), ib,
                                A(j, k), ldaj,
                                T(j, k), T.mb,
