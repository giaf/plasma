 ...
 *
 * @generated from test/test_zgels.c, normal z -> c, Sat Jul 22 09:25:14 2017
 *
 **/
#include "test.h"
#include "flops.h"
...
void test_cgels(param_value_t param[], bool run)
{
    ...
    plasma_cgels(PlasmaNoTrans, m, n, nrhs,
                 A, lda,
                 &T,
                 B, ldb);
    ...
    if (m >= n) {
        ...
        flop = flops_cgeqrf(m, n) + flops_cgeqrs(m, n, nrhs);
    }
    else {
        // cost of LQ-based factorization, triangular solve, and Q^H application
        flop = flops_cgelqf(m, n) +
               flops_ctrsm(PlasmaLeft, m, nrhs) +
               flops_cunmlq(PlasmaLeft, n, nrhs, m);
    }
