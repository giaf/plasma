 ...
 *
 * @generated from test/test_ztrtri.c, normal z -> s, Sat Jul 22 09:25:29 2017
 *
 **/
#include "test.h"
#include "flops.h"
...
void test_strtri(param_value_t param[], bool run)
{
    ...
    if (test) {
        ...
        float Inorm = LAPACK_slantr(&normc, &uploc, &diagc,
                                     &n, &n, Aref, &lda, work);
        ...
        float error = LAPACK_slantr(&normc, &uploc, &ndiagc,
                                     &n, &n, A, &lda, work);
        if (Anorm*Inorm != 0.0)
            error /= (Anorm * Inorm);
