 ...
 *
 * @generated from core_blas/core_ztradd.c, normal z -> c, Sat Jul 22 09:24:41 2017
 *
 **/
                ...
                plasma_complex32_t beta,        plasma_complex32_t *B, int ldb)
{
    ...
    if (uplo == PlasmaLower) {
        ...
        case PlasmaConjTrans:
            for (int j = 0; j < n; j++)
                for (int i = j; i < m; i++)
                    B[ldb*j+i] = beta * B[ldb*j+i] + alpha * conjf(A[lda*i+j]);
            break;
    ...
    plasma_sequence_t *sequence, plasma_request_t *request)
{
                     ...
                     depend(inout:B[0:ldb*n])
    {
        if (sequence->status == PlasmaSuccess) {
            int retval = core_ctradd(uplo, transa,
                                     m, n,
                                     alpha, A, lda,
                                     beta, B, ldb);
            if (retval != PlasmaSuccess) {
                plasma_error("core_ctradd() failed");
